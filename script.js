...
portfolioItems.addEventListener("click", function(event) {
    if (event.target.tagName.toLowerCase() == "button") {
        ...
        // Inject Vimeo SETELAH popup terbuka
        setTimeout(() => {
            const containers = popup.querySelectorAll(".video-container[data-vimeo]");
  ...
  <div class="vimeo-wrapper">
      <iframe
          src="https://player.vimeo.com/video/${id}?autoplay=0&muted=1&controls=1&loop=0"
...
function closePopupBox() {
    setTimeout(() => {
        popup.querySelectorAll('.video-container').forEach(v => {
            v.innerHTML = '';
            v.dataset.loaded = '';
        });
        popup.querySelector('.popup-body').innerHTML = '';
